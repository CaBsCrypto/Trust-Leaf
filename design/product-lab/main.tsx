...
function App() {
  const [direction, setDirection] = useState<'A' | 'B' | 'C'>('C');
  ...
  return <div className={`lab direction-${direction}`}>
    ...
    {page === 'attention' && direction !== 'A' && <ol className="steps" aria-label="Etapas de atención">{['Elegir paciente', 'Preparar entrega', 'Revisar'].map((label, i) => <li key={label} aria-current={step === i + 1 ? 'step' : undefined}><span>{i + 1 < step ? <Check size={15}/> : `0${i + 1}`}</span>{label}</li>)}</ol>}
