import { lotStateLabel, lotFilterLabel, lotReviewReason } from './lotPresentation';
...
function App() {
  ...
  return <div className={`lab direction-${direction} ${direction === 'E' ? 'direction-D desk-page-' + page : ''}`}>
    ...
    {page === 'inventory' && <>{SearchField}<div className="filters" role="group" aria-label="Estado del lote">{['Todos','Disponible','Cuarentena','Vencido'].map(s => <button key={s} aria-pressed={stockFilter === s} onClick={() => setStockFilter(s)}>{lotFilterLabel(s, direction === 'E')}</button>)}</div><div className="stock-list">{!filteredLots.length && <Empty text="No hay lotes para estos filtros."/>}{filteredLots.map(l => <article className="stock-row" key={l.id}><div className="lot-symbol"><Leaf size={24}/></div><div className="stock-name"><h2>{l.product}</h2><span>{l.id}</span></div><strong className="stock-quantity">{grams(l.stock)}</strong><span className={`badge ${l.state === 'Disponible' ? 'ok' : l.state === 'Vencido' ? 'danger' : 'warning'}`}>{lotStateLabel(l.state, direction === 'E')}{direction === 'E' && lotReviewReason(l) && <small className="desk-lot-reason">{lotReviewReason(l)}</small>}</span><span className="expiry">Vence<br/><strong>{l.expires}</strong></span><button onClick={() => openHistory(l.id)}>Ver historial<ArrowRight size={16}/></button></article>)}</div></>}
