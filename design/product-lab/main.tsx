...
import { ArrowLeft, ArrowRight, Check, ChevronRight, CircleAlert, ClipboardCheck, FileText, Home, Leaf, Menu, Package, Search, ShieldCheck, Users, X } from 'lucide-react';
...
function App() {
  ...
  useEffect(() => {
    ...
    if (page !== 'attention' || patientId || review) heading.current?.focus({ preventScroll: true });
  ...
  const title = page === 'home' ? 'Tu jornada, en orden.' : page === 'attention' ? review ? 'Revisar entrega' : patient ? patient.name : 'Pacientes autorizados' : pageLabel;
  ...
  const detail = patient && <section className="patient-detail" aria-label="Detalle del paciente">
    {direction === 'A' && <><div className="detail-top"><span className="avatar large">{patient.initials}</span><div><h2>{patient.name}</h2><span className="muted">{patient.id}</span></div></div><span className="permission"><ShieldCheck size={15}/>{patient.permission}</span></>}
  ...
  return <div className={`lab direction-${direction}`}>
    <aside className="lab-bar" aria-label="Controles del prototipo"><strong>LAB / TRUST LEAF</strong><span>Datos ficticios · sin guardado</span><div className="variant" role="group" aria-label="Dirección visual"><button aria-pressed={direction === 'A'} onClick={() => setDirection('A')}>A · Operativo</button><button aria-pressed={direction === 'B'} onClick={() => setDirection('B')}>B · Por tareas</button></div><label><span>Rol</span><select value={role} onChange={e => { const next = e.target.value; guarded(() => { clearDraft(); setRole(next); setPage(next === 'manager' ? 'home' : 'attention'); setPatientId(null); setSearch(''); setHistoryLot(''); setReceiptId(null); setViewState('ready'); }); }}><option value="manager">Encargado</option><option value="operator">Operador</option></select></label><label><span>Estado</span><select value={viewState} onChange={e => setViewState(e.target.value as ViewState)}><option value="ready">Contenido</option><option value="loading">Carga</option><option value="error">Error</option><option value="empty">Vacío</option></select></label></aside>
    <div className="app-shell"><aside className="navigation"><a className="brand" href="#" onClick={e => { e.preventDefault(); navigate(role === 'manager' ? 'home' : 'attention'); }}><Leaf size={26}/><span>trust leaf<span className="brand-caption">ESPACIO DE TRABAJO</span></span></a><button className="mobile-menu" aria-expanded={menu} aria-controls="main-nav" onClick={() => setMenu(!menu)}><Menu size={20}/>{pageLabel}</button><nav id="main-nav" aria-label="Navegación del dispensario" className={menu ? 'expanded' : ''}>{navigation.map(n => <button key={n.id} aria-current={page === n.id ? 'page' : undefined} onClick={() => navigate(n.id)}><n.icon size={19}/><span>{n.label}</span>{direction === 'B' && <ChevronRight size={14}/>}</button>)}</nav><div className="nav-foot"><ShieldCheck size={18}/><span>Piloto de pruebas<br/><small>Una sede · datos ficticios</small></span></div></aside>
    <div className="workspace"><header className="identity"><div><span className="site-mark">D</span><div><strong>Dispensario Demo</strong><small>Sede de prueba</small></div></div><div className="account"><span className="account-dot">{role === 'manager' ? 'EC' : 'OP'}</span><div><strong>{role === 'manager' ? 'Encargado' : 'Operador'}</strong><small>{role}@example.test</small></div></div></header>
    <main><div className="page-heading"><div><div className="eyebrow">{direction === 'A' ? 'OPERACIÓN DIARIA' : 'TU ESPACIO DE TRABAJO'} / {pageLabel}</div><h1 tabIndex={-1} ref={heading}>{title}</h1><p>{summary}</p></div>{page === 'home' && <button className="primary" onClick={() => navigate('attention')}>Iniciar atención<ArrowRight size={18}/></button>}</div>
    {page === 'attention' && direction === 'B' && <ol className="steps" aria-label="Etapas de atención">{['Elegir paciente', 'Preparar entrega', 'Revisar'].map((label, i) => <li key={label} aria-current={step === i + 1 ? 'step' : undefined}><span>{i + 1 < step ? <Check size={15}/> : `0${i + 1}`}</span>{label}</li>)}</ol>}
