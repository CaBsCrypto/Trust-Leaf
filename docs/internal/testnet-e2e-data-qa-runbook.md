...
- `trustleaf-testnet-deployer`: despliegue técnico.
- `trustleaf-testnet-admin`: inicialización, pausa y rollback.
- `trustleaf-testnet-doctor`: emisión/activación/revocación sintética.
- `trustleaf-testnet-dispensary`: parcial/dispensada sintética.
- `trustleaf-testnet-indexer`: solo lectura; nunca firma.
...
```powershell
stellar contract invoke --id receipt-ledger-testnet --source trustleaf-testnet-admin --network testnet -- --help
```
...
El **rollback owner** es quien controla `trustleaf-testnet-admin`, bajo doble revisión del responsable de seguridad. Debe pausar submissions, revocar grants/roles cuando sea seguro, poner adapter/indexer fail-closed y preservar evidencia. El contrato no se borra ni reescribe; una nueva instancia requiere otro hash, revisión y autorización explícita.
