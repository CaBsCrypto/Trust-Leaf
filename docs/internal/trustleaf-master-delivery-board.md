...
| Evidencia ReceiptLedger V1 existente | RPC/indexer + QA independiente | `integration/testnet-v2-readonly-durable-20260825` | lector RPC oficial read-only, allowlists exactas, attestation, cursor/restart/reorg/retry y reporte redactado; live cerrado por defecto | ejecución live acotada aprobada, revisión humana del resultado y backend durable administrado |
| Lector durable + UI read-only V2 | RPC/indexer + UX/QA + Scrum | `integration/testnet-v2-readonly-durable-20260825` | transporte RPC lazy con binding privado, attestation network/WASM/registry, journal local durable, port UI sanitizado y E2E; fixture por defecto | contract ID/deployment ledger/manifiesto V2, cuenta técnica allowlisted, DB durable/segunda fuente, adapter server-side autenticado y QA final |
| Flujo visual E2E sintético | UX/implementación + QA independiente + Scrum | `integration/testnet-v2-readonly-durable-20260825` | ruta aislada `/demo/pilot-flow`, 10 pasos, negativos visibles, QR público mínimo, preflight y Browser desktop/móvil verdes | pasada humana reduced-motion; IdP, datos durables y V2 siguen externos/NO-GO |
