...
- Status output is deliberately low-cardinality: booleans, mode, retry count and allowlisted safe codes. It excludes URLs, contract/account IDs, hashes, XDR, secrets, identities and clinical data. `submissionAttempts` is always zero. `ready=true` additionally requires a durable store and a materialized cursor; memory mode or `caught_up` without a cursor is never ready.
...
After configuration, run a read-only catch-up first. A PASS requires: network/WASM/TrustRegistry initialization attestation matched, runtime transport binding matched, a durable cursor persisted and recovered after restart, no gap/reorg unresolved, no unsafe report field, no submission attempt and independent comparison of a small opaque sample against an approved second Testnet source. A timeout, retention gap, unexpected event schema or bytecode/registry mismatch is `unknown`/NO-GO, never a reason to skip forward.
...
- `TRANSPORT_BINDING_REJECTED` or `DEPLOYMENT_LEDGER_REJECTED`: stop before RPC access; rebuild configuration from the reviewed deployment manifest.
