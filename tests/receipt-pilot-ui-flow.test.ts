...
receipt = applyReceiptPilotOperation(receipt, { kind: 'issue', operationKey: 'issue-1' });
assert.equal(receipt.state, 'active');
assert.equal(receipt.version, 1);
const replay = applyReceiptPilotOperation(receipt, { kind: 'issue', operationKey: 'issue-1' });
assert.deepEqual(replay, receipt, 'same operation must be idempotent');
receipt = applyReceiptPilotOperation(receipt, { kind: 'dispense-partial', units: 1, operationKey: 'partial-1' });
...
const invalid = applyReceiptPilotOperation(receipt, { kind: 'dispense-partial', units: 2, operationKey: 'over-1' });
assert.deepEqual(invalid, receipt, 'over-dispensation must fail closed');
receipt = applyReceiptPilotOperation(receipt, { kind: 'dispense-partial', units: 1, operationKey: 'partial-2' });
