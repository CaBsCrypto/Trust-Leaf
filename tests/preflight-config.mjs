...
for (const forbiddenLegacyName of ['STELLAR_ADMIN_SECRET', 'STELLAR_DOCTOR_SECRET', 'STELLAR_DISPENSARY_SECRET']) {
  assert.doesNotMatch(envExample, new RegExp(`^${forbiddenLegacyName}=`, 'm'), `${forbiddenLegacyName} must not be provisioned in .env.example`);
}
