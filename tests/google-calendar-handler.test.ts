...
let activeRole = 'doctor'; let saved = ''; let exchanges = 0;
...
activeRole='patient'; assert.equal((await run(request,'start')).code,403); activeRole='doctor';
...
assert.equal((await run({...callback,headers:{}},'callback')).location,'/medico?calendar=error');
assert.equal(exchanges,0);
assert.equal((await run(callback,'callback')).location,'/medico?calendar=connected');
assert.ok(saved && !saved.includes('refresh-test'));
assert.equal((await run(callback,'callback')).location,'/medico?calendar=error');
assert.equal(exchanges,1);
console.log('PASS: doctor authorization, browser binding, encrypted persistence and replay rejection');
