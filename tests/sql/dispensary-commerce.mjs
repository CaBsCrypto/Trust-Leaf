...
try {
  ...
  await assert.rejects(mutate('dispensary', 'save-product', { ...productInput, code: 'TOO-LONG', name: 'x'.repeat(101) }), { code: '23514' });
  ...
  const linkRead = await commerce('dispensary', 'batch-links', { batchRef: receipt.batchRef });
  assert.equal(linkRead.items[0].product_ref, p.resourceRef);
  assert.equal(linkRead.items[0].supplier_name, 'Synthetic supplier');
  assert.equal((await commerce('operator', 'batch-links', { batchRef: receipt.batchRef })).items[0].supplier_name, undefined);
  assert.equal((await commerce('dispensaryB', 'batch-links', { batchRef: receipt.batchRef })).items.length, 0);
  assert.equal((await commerce('dispensary', 'batch-links', { batchRef: randomUUID() })).items.length, 0);
  ...
  const oldBatch = await pilot('dispensary', 'receive-batch', { lotCode: 'LEGACY', product: 'Legacy synthetic', sourceReference: 'synthetic', expiresAt: '2099-01-01T00:00:00Z', quantityMg: 5000 });
  assert.equal((await commerce('dispensary', 'batch-links', { batchRef: oldBatch.resourceRef })).items[0].product_ref, null);
  const activeProduct = (await commerce('dispensary', 'products')).items.find(x => !x.archived);
  await mutate('dispensary', 'link-batch', { resourceRef: oldBatch.resourceRef, version: 1, productRef: activeProduct.product_ref, supplierRef: null });
  assert.equal((await commerce('dispensary', 'batch-links', { batchRef: oldBatch.resourceRef })).items[0].product_ref, activeProduct.product_ref);
  assert.equal((await call('dispensary', 'snapshot')).batches.find(x => x.batch_ref === oldBatch.resourceRef).stock_mg, 5000);
  ...
  await assert.rejects(commerce('operator', 'batch-links', { batchRef: receipt.batchRef }), { code: '42501' });
