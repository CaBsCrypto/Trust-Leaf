...
assert.equal(app.includes('if (hasRealAdminSession && adminAuth.user)'), true);
assert.equal(app.includes('<AdminReadinessPanel getIdToken={() => adminAuth.user!.getIdToken()} />'), true);
const realAdminStart = app.indexOf('if (hasRealAdminSession && adminAuth.user)');
const legacyAdminStart = app.indexOf('<AdminRoute', realAdminStart);
const realAdminBlock = app.slice(realAdminStart, legacyAdminStart);
assert.equal(realAdminBlock.includes('<AdminRoute'), false, 'real admin must not receive legacy mutation controls');
