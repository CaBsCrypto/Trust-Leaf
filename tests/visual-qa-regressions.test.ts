...
assert.match(adminGate, /const permissionStatusText\s*=\s*usingPrivy\s*\? 'Permiso administrativo requerido en Supabase'\s*:\s*authState\.mode/, 'Privy must describe required server permissions before considering any legacy auth state');
assert.match(adminGate, /\[usingPrivy \? 'Permisos' : 'Allowlist', permissionStatusText\]/, 'Privy must not label its authorization as the legacy document allowlist');
assert.match(adminGate, /payload\.authorized && payload\.role === 'admin'/, 'display copy must not replace the server admin authorization check');
