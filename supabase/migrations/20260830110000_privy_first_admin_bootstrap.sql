-- First-admin ceremony for a verified Privy DID. It accepts no clinical data
-- and is callable only by the server's service-role key after its email gate.

begin;

create function trustleaf_private.bootstrap_first_privy_admin(subject text)
returns table (
  actor_ref uuid,
...
begin
  if subject is null or subject !~ '^did:privy:[A-Za-z0-9._:-]{6,500}$' then
    raise exception 'PRIVY_SUBJECT_INVALID' using errcode = '22023';
  end if;

  -- One global winner; a retry by the same account is idempotent.
  ...
  if exists (select 1 from actor_bindings where role = 'admin') then
...
comment on function trustleaf_private.bootstrap_first_privy_admin(text)
is 'One-time first-admin binding for a server-verified Privy DID. Server enforces the email allowlist.';
