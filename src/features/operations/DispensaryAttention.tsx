...
export default function DispensaryAttention({ data, search, onSearch, disabled, readError, receiptRef, submit, history, onDirtyChange }: {
  data: PilotSnapshot; search: string; onSearch: (value: string) => void; disabled: boolean; readError: string; receiptRef: string | null;
  ...
  const lastReceipt = (data.deliveries ?? []).filter(d => d.treatment_ref === treatmentRef).sort((a,b) => b.created_at.localeCompare(a.created_at))[0];
  ...
  const mayLeave = async () => !dirty || await confirmDiscard('La preparacion de entrega se perdera. No se registrara ninguna entrega.');
  async function back() {
    if (disabled || !await mayLeave()) return;
  ...
  return <div className={`op-attention ${selected && treatment ? 'op-attention-selected' : ''}`}>
    ...
    <section className="op-patient-detail" aria-label="Detalle del paciente">
      ...
      {treatment && period && <>
        ...
        {!receipt && lastReceipt && <details><summary>Ultimo comprobante · {formatGrams(lastReceipt.quantity_mg)}</summary><p>{lastReceipt.product ?? 'Producto no disponible'} · {lastReceipt.lot_code ?? 'Lote no disponible'}</p><p className="op-reference">{lastReceipt.delivery_ref}</p><button className="op-command" onClick={history}>Ver historial</button></details>}
