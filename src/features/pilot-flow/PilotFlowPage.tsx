...
import { PILOT_PHASES, PILOT_QR_HANDLE } from './pilotFlowFixtures';
...
const NEXT_ACTION: Record<string, { label: string; action: PilotAction }> = {
  ...
  'appointment-booked': { label: 'Registrar gates sintéticos', action: { type: 'doctor-complete-consultation', actor: 'doctor', consent: true, syntheticEligible: true } },
    ...
    <div className="min-h-screen bg-[#f2f5f1] text-[#102a22]" data-pilot-phase={state.phase}>
      ...
      <main id="pilot-main" className="mx-auto max-w-7xl space-y-5 px-4 py-5 sm:px-6 sm:py-8">
        ...
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1.4fr)_minmax(18rem,.6fr)]">
          <section className="rounded-[2rem] border border-[#102a22]/10 bg-white p-6 shadow-sm sm:p-8" aria-labelledby="current-title">
            ...
            {(state.phase === 'directory-enabled' || state.phase.startsWith('dispense')) && <div className="mt-5 rounded-2xl bg-[#eef7f2] p-4"><p className="text-xs font-bold uppercase tracking-widest text-[#315c4e]">Constancia QR mínima</p><p className="mt-2 font-mono text-sm font-bold">{safeQr}</p><p className="mt-2 text-xs text-[#315c4e]">Coincidencia local, estado y versión. Sin identidad, detalle, cantidad o historial.</p></div>}
        ...
        <div className="flex justify-center pb-8"><button type="button" onClick={() => { dispatch({ type: 'reset', actor: 'admin' }); setActiveRole('admin'); }} className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-[#102a22]/20 bg-white px-4 text-sm font-bold"><RotateCcw size={17} aria-hidden="true" /> Reiniciar fixtures</button></div>
      </main>
    </div>
  );
}
